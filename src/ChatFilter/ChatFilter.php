...
class ChatFilter {
	 ...
	 * @return string
	 */
	private function getErrorMessage(string $message, Player $player): string {
		$errorMsg = '';

		if (strlen($message) === 0) {
			$errorMsg = TextFormat::RED . '[LBCF] That message is too short.';
		} elseif (isset($this->recentChat[$player->getID()])) {
 			/* player already posted message in last 3 seconds */
 			$errorMsg = TextFormat::RED . '[LBCF] You are messaging too fast.';
 		} elseif (isset($this->recentMessages[$player->getID()]) &&
				$this->recentMessages[$player->getID()] === $message) {
			/* player's message repeated his previous message */
			$errorMsg = TextFormat::RED . '[LBCF] You repeated that message.';
		} elseif ($this->profanityChecker->getIsProfane()) {
			$errorMsg = TextFormat::RED . '[LBCF] That\'s an inappropriate message.';
		} elseif ($this->profanityChecker->getIsDating()) {
			$errorMsg = TextFormat::RED . '[LBCF] No dating.';
		} elseif ($this->profanityChecker->getIsAdvertising()) {
			$errorMsg = TextFormat::RED . '[LBCF] No advertising';
		}
